package com.zhai.picshared.manager.upload;
...
import com.zhai.picshared.config.CosClientConfig;
...
public abstract class PictureUploadTemplate {  
    ...
    public final UploadPictureResult uploadPicture(Object inputSource, String uploadPathPrefix) {
        ...
        try {  
            ...
            List<CIObject> ciObjects = processResults.getObjectList();
            if(ciObjects != null) {
                CIObject compressedObject = ciObjects.get(0);
                CIObject thumbnailObject = compressedObject;
                if(ciObjects.size() > 1) {
                    thumbnailObject = ciObjects.get(1);
                }
                return buildResult(originFilename, compressedObject, thumbnailObject,imageInfo);
            }
            // 5. 封装返回结果
